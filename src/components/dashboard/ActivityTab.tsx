...
import type { CallLog, Client, SmsLog } from '@/types/respondfall';

type MergedCall = CallLog & { _t: 'call'; _ts: string };
type MergedSms = SmsLog & { _t: 'sms'; _ts: string };
type MergedItem = MergedCall | MergedSms;
...
export function ActivityTab({ client }: { client: Client }) {
  const { callLogs, smsLog, simulateCall, setConfirmDel, vmailOpen, toggleVmail, activityLoading } =
    useAppStore();
  ...
  const handleTestSetup = async () => {
    ...
    try {
      ...
      if (error || !data?.ok) {
        ...
        toast.error(
          `Twilio connection failed: ${msg}. Check your credentials in Settings → Connect.`
        );
        return;
      }
  ...
  const merged: MergedItem[] = [
    ...
    ...smsLog
      ...
      .map((s) => ({ ...s, _t: 'sms' as const, _ts: s.sent_at })),
  ...
  return (
    <div>
      {/* Sim bar */}
      <div
        className="bg-s1 border border-ember rounded-[10px] p-3 px-3 sm:px-4 flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2.5"
        style={{
      ...
      >
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button
            className="bg-[hsl(var(--destructive)/0.08)] text-destructive border border-destructive/20 rounded-[7px] py-1.5 px-3 cursor-pointer text-[11px] font-mono flex items-center gap-1 hover:bg-[hsl(var(--destructive)/0.15)] transition-all"
            onClick={() => setConfirmDel({ type: 'activity', id: 'all', label: 'all activity' })}
          >
            🗑 Clear
          </button>
          <button
      ...
      {activityLoading && merged.length === 0 ? (
        <div className="space-y-2">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="bg-s1 border border-blue rounded-[10px] p-3 sm:p-4 animate-pulse h-[72px]"
            />
          ))}
        </div>
      ) : merged.length === 0 ? (
        <div className="text-center py-12 px-5 bg-s1 border border-dashed border-blue-2 rounded-xl">
          ...
          <div className="text-[15px] font-display font-bold text-foreground mb-2">
            You're live — waiting for your first call
          </div>
          <div className="text-[12px] text-t3 font-mono leading-relaxed max-w-md mx-auto mb-4">
            When someone calls your Respondfall number and hangs up or goes to voicemail, it will
            appear here instantly. Your automated SMS sequence fires within seconds.
          </div>
          <button
        ...
        merged.map((item) => {
          ...
          const smsItem = item._t === 'sms' ? item : null;
          const callItem = item._t === 'call' ? item : null;
          const step = smsItem?.step ?? null;
          ...
          const hasVmail = callItem !== null && callItem.voicemail && callItem.voicemail_transcript;
          const vmId = `vm-${item.id}`;

          return (
            <div
              key={item.id}
              className="bg-s1 border border-blue rounded-[10px] p-3 px-3 sm:px-4 grid grid-cols-[32px_1fr_auto] sm:grid-cols-[38px_1fr_auto] gap-2 sm:gap-3 items-start animate-fade-up mb-2 hover:border-blue-2 transition-colors"
            >
              <div
                className={`w-8 h-8 sm:w-[38px] sm:h-[38px] rounded-lg flex items-center justify-center text-[15px] sm:text-[17px] flex-shrink-0 mt-0.5 border ${iconClass}`}
              >
                {icon}
              </div>
              <div className="min-w-0">
                <div className="font-mono text-[12px] sm:text-[13px] font-medium text-foreground mb-0.5 truncate">
                  {smsItem ? smsItem.to_number : callItem?.caller_number}
                </div>
                <div className="text-[11px] text-t2 leading-relaxed line-clamp-2">
                  {smsItem ? (
                    <>
                      {stepLabel && (
                        <span
                          className={`font-semibold ${isReview ? 'text-gold' : isAi ? 'text-purple-brand' : 'text-sky'}`}
                        >
                          [{stepLabel}]
                        </span>
                      )}{' '}
                    ...
                    `Missed call (${callItem?.call_status}) · sequence triggered`
                  )}
                </div>
                {hasVmail && (
                  <div className="mt-2 bg-3 border border-purple/30 rounded-lg overflow-hidden">
                    <div
                      className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-purple-bg/50 transition-colors"
                      onClick={() => toggleVmail(vmId)}
                    >
                      <span className="text-[11px] font-mono text-purple-brand tracking-[.06em] flex items-center gap-1.5">
                        🎙 Voicemail
                      </span>
                    ...
                    {vmailOpen[vmId] && (
                      <div className="px-3 py-2.5 border-t border-purple/20">
                        <div className="text-[9px] font-mono text-purple-brand uppercase tracking-[.1em] mb-1.5">
                          AI Transcript
                        </div>
                        <div className="italic text-foreground text-xs leading-relaxed border-l-2 border-purple-brand/50 pl-2.5">
                          {callItem?.voicemail_transcript}
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex flex-col items-end gap-1">
                <div className="font-mono text-[9px] sm:text-[10px] text-t3 whitespace-nowrap">
                  {formatTime(item._ts)}
                </div>
                {(() => {
                  const status = smsItem?.status ?? null;
                    ...
                    : isFailed
                      ...
                      : isDelivered
                        ...
                        : isReview
                          ...
                          : isAi
                            ? 'bg-purple-bg text-purple-brand border-purple'
                            : step === 'qual'
                  ...
                  return (
                    <span
                      className={`text-[9px] sm:text-[10px] font-semibold font-mono px-1.5 sm:px-2 py-0.5 rounded border ${badgeCls}`}
                    >
                      {label}
                    </span>
                  );
                ...
                <button
                  ...
                  onClick={() =>
                    setConfirmDel({ type: 'feed-item', id: item.id, label: 'this entry' })
                  }
                >
                  🗑
                </button>
