...
const intentTag = (i: string) => {
  switch (i) {
    ...
    case 'service': return 'bg-success-bg text-success border-success';
    case 'appointment': return 'bg-success-bg text-success border-success';
    case 'referral': return 'bg-purple-bg text-purple-brand border-purple';
    ...
    <div>
      ...
      {activityLoading && convos.length === 0 ? (
        ...
        convos.map(cv => {
          ...
          const hasReferralSent = cv.messages.some(m => m.step === 'referral' && m.direction === 'outbound');
            ...
            <div key={cv.phone} className={`bg-s1 border rounded-xl p-3 sm:p-4 mb-3 transition-colors animate-fade-up ${cv.hasInbound ? 'border-ember' : revSent ? 'border-gold' : 'border-blue'}`}>
              <div className="flex justify-between items-start mb-3 gap-2">
                ...
                <div className="flex gap-1.5 items-start flex-shrink-0 flex-wrap justify-end">
                  {qualFlow && (
                    <span className={`text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 rounded border ${
                      qualFlow.stage === 'routed'
                        ? qualFlow.routedTo === 'booking' ? 'bg-success-bg text-success border-success' : 'bg-ember-dim text-ember border-ember'
                        : 'bg-amber-500/10 text-amber-500 border-amber-500/30'
                    }`}>
                      {qualFlow.stage === 'routed'
                        ? qualFlow.routedTo === 'booking' ? '📅 ROUTED' : '📋 NOTIFIED'
                        : '⏳ QUALIFYING'}
                    </span>
                  )}
                  {cv.intents.map(i => (
              ...
              {phoneReferrals.length > 0 && (
                <div className="mb-3 space-y-1.5">
                  {phoneReferrals.map(ref => (
                    <div key={ref.id} className="bg-purple-bg border border-purple rounded-lg px-3 py-2 text-xs flex items-center justify-between gap-2">
                      <div>
                        <span className="text-purple-brand font-semibold">🤝 Referral:</span>{' '}
                        <span className="text-foreground">{ref.referredName}</span>
                        {ref.referredPhone && <span className="text-t3 ml-1.5 font-mono">{ref.referredPhone}</span>}
                      </div>
                ...
                <>
                  ...
                  <div className="flex gap-1.5 mt-2.5 flex-wrap">
                    ...
                    {revSent && !hasReferralSent && (
                      <button className="text-[10px] sm:text-[11px] font-mono py-1.5 px-2.5 rounded-md cursor-pointer border border-purple bg-transparent text-purple-brand hover:bg-purple-bg transition-all flex items-center gap-1" onClick={() => sendReferralRequest(cv.phone)}>
                        🤝 Ask for Referral
                      </button>
                    )}
