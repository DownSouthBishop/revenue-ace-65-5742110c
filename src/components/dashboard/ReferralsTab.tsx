...
export function ReferralsTab({ client: _client }: { client: Client }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : 'Failed to send referral request');
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2 lg:gap-3">
        {[
          ...
          <div
            key={i}
            className="bg-s1 border border-blue rounded-xl p-3 lg:p-4 relative overflow-hidden"
          >
      ...
      <div className="bg-s1 border border-blue rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-blue flex items-center justify-between">
          <div className="font-display text-sm font-bold tracking-[.04em] text-foreground">
            Referral Activity
          </div>
          <div className="text-[10px] font-mono text-t3 uppercase tracking-[.12em]">
        ...
        {sorted.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <div className="text-xs text-t3 max-w-md mx-auto">
              After marking a job complete, you can send a referral request to that customer from
              their conversation in Inbox.
            </div>
          </div>
        ) : (
            ...
            {sorted.map((r) => (
              <div key={r.id} className="px-4 py-3 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span
                    className={`text-[9px] font-mono uppercase tracking-[.1em] border rounded-full px-2 py-0.5 ${STATUS_STYLES[r.status] || ''}`}
                  >
                    {r.status}
                  </span>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-4">
        <div className="font-display text-sm font-bold tracking-[.04em] text-foreground mb-1">
          Send Referral Request
        </div>
        <div className="text-[11px] font-mono text-t3 mb-3">
          Send a customer the referral SMS to capture a new lead.
        </div>
