...
export const SettingsTab = forwardRef<HTMLDivElement, { client: Client }>(function SettingsTab(
  { client },
  ref
) {
  ...
  const save = async () => {
    const { error } = await updateClient(client.id, {
      ...
      forward_from_number: fwdNum,
      timezone: tz,
      is_active: active,
      daily_sms_cap: cap,
      forward_timeout_seconds: fwdTimeout,
    });
    if (error) {
    ...
    <div ref={ref}>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          <div>
            <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">
              Timezone (for blackout)
            </label>
            <select
              className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none cursor-pointer"
              value={tz}
          ...
          <div>
            ...
            <input
              ...
              min={10}
              max={5000}
              className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
              value={cap}
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
              value={fwdTimeout}
              onChange={(e) => setFwdTimeout(parseInt(e.target.value) || 18)}
            />
          </div>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <button
            onClick={() => exportLeadsCSV(client.id, client.name)}
            aria-label="Export leads as CSV"
            className="bg-s2 border border-blue rounded-md text-t2 text-[11px] font-mono px-3 py-2 cursor-pointer hover:text-sky hover:border-primary transition-all"
          >
            ⬇ Export Leads CSV
          </button>
          <button
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          <div>
            ...
            <select
              className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none cursor-pointer"
              value={type}
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary"
              value={jobVal}
              onChange={(e) => setJobVal(parseInt(e.target.value) || 0)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        ...
        <div className="mb-3.5">
          ...
          <div className="font-mono text-base font-semibold text-success p-2.5 px-3 bg-3 border border-success rounded-lg">
            {client.twilio_phone_number}
          </div>
        </div>
        <div className="mb-3.5">
          <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">
            Your Business Number <span className="text-t3">(calls forward from here)</span>
          </label>
          <input
            className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary"
            value={fwdNum}
          ...
          />
          <div className="text-[11px] text-t3 font-mono mt-1">
            Enter the number your customers call. Missed calls will forward to your Respondfall
            number above.
          </div>
        </div>
        <div className="bg-sky-dim border border-blue-2 rounded-lg p-3 text-xs text-t2 mb-3.5 leading-relaxed border-l-[3px] border-l-primary">
          To change your Respondfall number, search and claim a new one below.
        </div>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        ...
        <div className="flex flex-col gap-3.5">
          <div>
            ...
            <textarea
              className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary min-h-[78px] resize-y"
              value={template}
                ...
                <div className="mt-2.5">
                  ...
                  <div className="bg-[hsl(var(--muted))] rounded-2xl p-3 sm:p-4">
                    <div className="bg-white text-neutral-900 rounded-2xl rounded-bl-md px-3.5 py-2.5 text-[13px] leading-snug max-w-[320px] shadow-sm">
                      {previewText || (
                        <span className="text-neutral-400">Your message will appear here…</span>
                      )}
                    </div>
                  </div>
                  <div className={`text-[11px] font-mono mt-1.5 text-right ${counterCls}`}>
                    {len}/160 characters
                  </div>
                </div>
              );
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
                value={bStart}
                onChange={(e) => setBStart(parseInt(e.target.value) || 0)}
              />
            </div>
            <div>
              <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">
              ...
              <input
                ...
                className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
                value={bEnd}
                onChange={(e) => setBEnd(parseInt(e.target.value) || 0)}
              />
            </div>
          </div>
        </div>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        ...
        <div className="flex flex-col gap-3.5">
          <div>
            <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">
              Booking Link <span className="text-ember">★ Critical</span>
            </label>
            <input
              className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
              value={bookLink}
          ...
          <div>
            <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">
              Google Review Link <span className="text-gold">★ Post-job review requests</span>
            </label>
            <input
              className="w-full bg-3 border border-blue rounded-lg text-foreground text-[13px] px-3 py-2.5 outline-none focus:border-primary"
              value={reviewLink}
            ...
            {!reviewLink && (
              <div className="bg-[hsl(var(--warning-bg))] border border-[hsl(var(--warning-border))] rounded-lg p-2.5 text-xs text-[hsl(var(--warning))] mt-2 leading-relaxed">
                Without this, post-job review requests are disabled. Add your Google review link to
                unlock.
              </div>
            )}
          </div>
      ...
      <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
        <div className="font-display text-base font-bold tracking-[.05em] mb-4 text-destructive flex items-center gap-2.5">
          <span className="w-[3px] h-4 bg-destructive rounded-sm" />
          Danger Zone
        </div>
        <div className="text-[13px] text-t2 mb-3.5">
          Sign out of your account, or permanently remove this client and all associated data.
        </div>
        <div className="flex flex-col gap-2.5">
          ...
          <button
            className="bg-[hsl(var(--destructive)/0.08)] text-destructive border border-destructive/20 rounded-[7px] py-2 px-4 cursor-pointer text-[13px] font-mono flex items-center gap-1.5 hover:bg-[hsl(var(--destructive)/0.15)] transition-all"
            onClick={() => setConfirmDel({ type: 'client', id: client.id, label: client.name })}
          >
            🗑 Delete Client
          </button>
        </div>
      ...
      <div className="flex items-center gap-3.5">
        <button
          className="gradient-sky text-primary-foreground border-none rounded-lg py-2.5 px-6 font-display text-sm font-bold tracking-[.06em] uppercase cursor-pointer glow-sky hover:-translate-y-px transition-all active:scale-[0.98]"
          onClick={save}
        >
          SAVE CHANGES
        </button>
