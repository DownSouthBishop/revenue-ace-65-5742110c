  ...
  return (
    <div className="bg-s1 border border-blue rounded-xl p-5 mb-3.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <div>
          <div className="text-t3 font-mono uppercase text-[10px] mb-1">Status</div>
          <div className={ok ? 'text-success' : 'text-ember'}>{ok ? '● Healthy' : '● Issues'}</div>
        </div>
        <div>
          <div className="text-t3 font-mono uppercase text-[10px] mb-1">Last webhook</div>
          <div className="text-foreground">
            {h?.last_webhook_ping ? new Date(h.last_webhook_ping).toLocaleString() : '—'}
          </div>
        </div>
        <div>
          <div className="text-t3 font-mono uppercase text-[10px] mb-1">Last send</div>
          <div className="text-foreground">
            {h?.last_successful_send ? new Date(h.last_successful_send).toLocaleString() : '—'}
          </div>
        </div>
        <div>
          <div className="text-t3 font-mono uppercase text-[10px] mb-1">Pending follow-ups</div>
...
export async function exportLeadsCSV(clientId: string, businessName: string) {
  ...
  (calls.data ?? []).forEach((c: MissedCallRow) =>
    rows.push([
      'missed_call',
      c.caller_number,
      '',
      (c.voicemail_transcript || '').replace(/\n/g, ' '),
      c.called_at ?? '',
    ])
  );
  (msgs.data ?? []).forEach((m: MessageRow) =>
    rows.push([
      'sms',
      m.caller_number ?? '',
      m.direction ?? '',
  ...
  a.href = url;
  a.download = `${businessName.replace(/\W+/g, '_')}_leads_${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
