    ...
    () => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const reasonMap: Record<string, QualReason> = {
            '1': 'quote',
            '2': 'service',
            '3': 'question',
          };
...
function runSimulatedQualReply(store: Store, phone: string, text: string) {
  ...
  useAppStore.setState((s) => ({
    ...
    qualFlows: s.qualFlows.map((q) =>
      q.phone === phone
        ? { ...q, stage: 'follow_up_1' as const, reason, answers: [...q.answers, text] }
        : q
    ),
  }));
  ...
  setTimeout(() => {
    const responses: Record<QualReason, string[]> = {
      quote: [
        'Kitchen sink leak — water damage starting under the cabinet',
        'Full bathroom remodel, two bathrooms',
        'Water heater replacement, current one is 15 years old',
      ],
      service: [
        'Thursday afternoon works great',
        'Tomorrow morning if possible, before 10am',
        'Any day this week after 2pm',
      ],
      question: [
        'Do you offer free estimates?',
        "What's your hourly rate for emergency calls?",
...
function storeSimulatedReferralResponse(
  store: Store,
  phone: string,
  name: string,
