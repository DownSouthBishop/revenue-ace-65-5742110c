    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-[600px]">
        <div className="text-center mb-6">
          <h1 className="font-display text-[26px] font-bold tracking-[.06em] text-foreground">
        ...
        <div className="flex gap-2 mb-6">
          {STEPS.map((name, i) => (
            <div key={name} className="flex-1">
              ...
              <div className={`text-[10px] font-mono text-center tracking-[.07em] transition-colors duration-400 ${i <= obStep ? 'text-sky' : 'text-t3'}`}>{name}</div>
            </div>
          ))}
        </div>

        {/* Card */}
        <div className="bg-s1 border border-blue rounded-2xl p-7 relative overflow-hidden animate-fade-up">
          <div className="absolute top-0 left-0 right-0 h-0.5 gradient-shimmer" />

          {obStep === 0 && (
            <div>
              <div className="flex items-center gap-2.5 font-display text-lg font-bold tracking-[.05em] mb-5">
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div className="col-span-2">
                  ...
                  <input className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary focus:shadow-[0_0_0_3px_hsl(var(--sky-dim))]" value={obForm.name} onChange={e => setObForm({ name: e.target.value })} placeholder="Miami Plumbing Co." />
                </div>
                <div>
                  <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">Industry</label>
                  <select className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none cursor-pointer focus:border-primary" value={obForm.business_type} onChange={e => setObForm({ business_type: e.target.value })}>
                ...
                <div>
                  ...
                  <input type="number" className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary" value={obForm.avg_job_value} onChange={e => setObForm({ avg_job_value: parseInt(e.target.value) || 0 })} />
                </div>
                <div className="col-span-2">
                  ...
                  <input className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary" value={obForm.booking_link} onChange={e => setObForm({ booking_link: e.target.value })} placeholder="https://cal.com/yourbusiness" />
                </div>
                <div className="col-span-2">
                  ...
                  <input className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary" value={obForm.google_review_link} onChange={e => setObForm({ google_review_link: e.target.value })} placeholder="https://g.page/r/.../review" />
                </div>
              </div>
            </div>
          )}
          ...
          {obStep === 1 && (
            <div>
              ...
              <div className="bg-sky-dim border border-blue-2 rounded-lg p-3 text-[12px] text-t2 mb-4 leading-relaxed border-l-[3px] border-l-primary">
                <strong className="text-sky">No external accounts needed.</strong> SkyforgeAI provisions your number instantly. Search by area code or city.
              </div>
              ...
              {obForm.selectedPhoneNumber && (
                <div className="flex items-center gap-2.5 p-3 bg-success-bg border border-success rounded-lg mt-2">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] font-mono text-t3">Provisioned · SkyforgeAI Infrastructure</div>
                  </div>
                </div>
              )}
              <div className="mt-3.5">
                <label className="text-[10px] font-mono text-t3 uppercase tracking-[.1em] block mb-1.5">Client's Existing Number <span className="text-t3">(for forwarding)</span></label>
                <input className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary" value={obForm.forward_from_number} onChange={e => setObForm({ forward_from_number: e.target.value })} placeholder="+13055559999" />
              </div>
            </div>
          )}
          ...
          {obStep === 2 && (
            <div>
              ...
              <div className="mb-3">
                ...
                <textarea className="w-full bg-3 border border-blue rounded-lg text-foreground font-body text-[13px] px-3 py-2.5 outline-none focus:border-primary min-h-[90px] resize-y leading-relaxed" value={obForm.sms_template} onChange={e => setObForm({ sms_template: e.target.value })} />
          ...
          {obStep === 3 && (
            <div>
              ...
              <div className="bg-sky-dim border border-blue-2 rounded-lg p-3 text-[12px] text-t2 mb-4 leading-relaxed border-l-[3px] border-l-primary">
                Set up conditional forwarding so missed calls route to your Respondfall number. <strong className="text-sky">Only unanswered calls</strong> forward.
              </div>
              ...
              <div className="bg-ember-dim border border-ember rounded-lg p-3 text-[12px] text-t2 leading-relaxed border-l-[3px] border-l-accent">
                ...
                <span className="font-mono text-[13px] text-ember">*61*+1XXXXXXXXXX*11*20#</span>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2.5 mt-6">
            {obStep > 0 && obStep < 3 && (
              <button className="flex-1 py-2.5 rounded-lg border border-blue-2 bg-transparent text-t2 font-display text-sm font-bold tracking-[.06em] uppercase cursor-pointer hover:bg-s2 hover:text-foreground transition-all active:scale-[0.98]" onClick={() => setObStep(obStep - 1)}>← Back</button>
            )}
            <button className="flex-[2] py-2.5 rounded-lg gradient-sky text-primary-foreground border-none font-display text-sm font-bold tracking-[.06em] uppercase cursor-pointer glow-sky hover:-translate-y-px transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]" onClick={handleNext} disabled={saving}>
