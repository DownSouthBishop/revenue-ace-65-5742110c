...
const clientToRow = (c: Partial<Client>): Partial<ClientInsert> => {
  ...
  if (c.forward_timeout_seconds !== undefined)
    row.forward_timeout_seconds = c.forward_timeout_seconds;
  return row;
};
  ...
  persist(
    (set, get) => ({
      ...
      obForm: {
        ...
        google_review_link: '',
        sms_template:
          'Hey, {business_name} here — sorry we missed you! Book a time: {booking_link}. Reply STOP.',
        selectedPhoneNumber: '',
        forward_from_number: '',
        blackout_start: 22,
        blackout_end: 7,
      ...
      setActiveClientId: (id) =>
        set({ activeClientId: id, tab: 'activity', mobileMenuOpen: false }),
      tab: 'activity',
      ...
      loadActivityForClient: async (clientId: string) => {
        if (!clientId) {
          set({ callLogs: [], smsLog: [], optOuts: [], activityLoading: false });
          return;
        }
            ...
            (payload) => {
              ...
              set((s) => {
                ...
                if (
                  ...
                  isStopKeyword(log.body) &&
                  !s.optOuts.includes(log.from_number)
                ) {
              ...
              if (log.direction === 'inbound') {
                try {
                  if (
                    ...
                    new Notification('New SMS reply', {
                      body: `${log.from_number}: ${log.body.slice(0, 80)}`,
                      icon: '/icon-192.png',
                      tag: `sms-${log.from_number}`,
                    });
                  }
                } catch {}
              }
      ...
      loadClients: async () => {
        ...
        set((s) => ({
          clients,
          activeClientId:
            clients.find((c) => c.id === s.activeClientId)?.id || clients[0]?.id || '',
        }));
      },
      ...
      addClient: async (c) => {
        ...
        const { data, error } = await supabase.from('clients').insert(initialRow).select().single();
        if (error) {
          console.error('addClient', error);
          return { client: null, error: error.message || 'Could not save client.' };
        }
        let newClient = rowToClient(data);
        if (wantedNumber) {
          const { data: buy, error: buyErr } = await supabase.functions.invoke(
            'twilio-buy-number',
            {
      ...
      deleteClient: async (id) => {
        ...
        if (client?.twilio_number_sid) {
          try {
            await supabase.functions.invoke('twilio-release-number', {
              body: { numberSid: client.twilio_number_sid },
            });
          } catch (e) {
            console.warn('twilio release failed', e);
          }
        }
        ...
        set((s) => {
          ...
          if (remaining.length === 0)
            return { clients: [], page: 'onboard' as PageId, obStep: 0, activeClientId: '' };
      ...
      storeReferralResponse: (phone: string, name: string, referredPhone?: string) => {
        ...
        supabase
          ...
          .then(({ error }) => {
            if (error) console.warn('referral insert failed', error);
          });
      },
      ...
      setReplyText: (phone, text) =>
        set((s) => ({ replyTexts: { ...s.replyTexts, [phone]: text } })),
      ...
      searchPhoneNumbers: async (query) => {
        ...
        try {
          ...
          const results: PhoneNumber[] = data.numbers.map(
            (n: { number: string; locality: string; region: string }) => ({
              number: n.number,
              locality: n.locality,
              region: n.region,
              price: '',
